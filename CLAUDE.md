...
- **Measure diarization with `diarize::quality::accuracy_against_reference`, never by eye.** It builds a reference from the embedder alone (hand-verified voice samples → per-line nearest match) and reports the majority-class rate alongside, so a number a constant predictor could reach is visible as one. Current state on a 12-minute two-person podcast: **84.1 % over 107 scorable lines, majority baseline 60.7 %** — but the pipeline emits **five clusters for two speakers**, which is the open problem. Tried and measured as *not* helping: raising `CLUSTER_DIST` inside the measured band, longer `MIN_ANCHOR_S` (4 s, 8 s), a lower `MAX_JOINED_S` (4/8/12 s), and a switch penalty above 0.2. The remaining error is not assignment noise — the clusters themselves do not correspond to the voices.
