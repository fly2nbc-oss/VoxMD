...
- **Tune `CLUSTER_DIST` by measuring, never by fitting the cluster count.** `diarize::tests::embedding_distance_matrix` embeds stretches whose speaker is known from the transcript and prints the pairwise distances: same voice 0.07–0.18, different voices 0.55–0.67. The constant belongs in that empty band. Fitting it against the *old* window-chopped segmentation suggested 0.65 — above the band, merging every speaker into one cluster, while still reporting a plausible-looking two-cluster split. Segmentation first, then this.
...
- **Measure diarization with `diarize::quality::accuracy_against_reference`, never by eye.** It builds a reference from the embedder alone (hand-verified voice samples → per-line nearest match) and reports the majority-class rate alongside, so a number a constant predictor could reach is visible as one. Current state on a 12-minute two-person podcast: **90.7 % over 107 scorable lines, majority baseline 60.7 %**. Tried and measured as *not* helping, so do not retry them blind: raising `CLUSTER_DIST` beyond the measured band (0.70 gives two clusters and 60.7 %, i.e. the baseline — it merges the wrong pair), longer `MIN_ANCHOR_S`, a fixed-length centred excerpt for embedding (3 s collapses everything to one cluster), tolerating any overlapped speech in anchors, and a Viterbi switch penalty above 0.2.
- **A better embedding model would not have helped, and is not the next step.** `wespeaker_en_voxceleb_CAM++` separates two voices cleanly — 0.07–0.18 within a speaker, 0.55–0.67 across — measured on this very audio. Every failure so far came from feeding it mixed audio, not from the model. `pyannote/wespeaker-voxceleb-resnet34-LM` (what pyannote 3.1 uses) is ungated on HF but ships PyTorch only, so adopting it means an ONNX conversion for no demonstrated gain.
- **The sibling `HMS-Transkription-App` does this properly and is the reference to compare against.** It shells out to **WhisperX** (`--diarize`), which brings three things this module does not have: the complete, calibrated `pyannote/speaker-diarization-community-1` pipeline instead of the raw `segmentation-3.0` ONNX model plus a hand-rolled clusterer; **word-level forced alignment** before diarization, so speakers attach to words rather than to Whisper's loose segment timestamps (`speakers_for_lines` maps turns onto whole lines, which is why a one-second interjection lands on the wrong speaker); and licensed access to the gated pyannote weights via an HF token. VoxMD deliberately avoids the token — that is the tradeoff, and the quality gap is its price. Before investing further here, decide whether to close that gap or to accept it.
