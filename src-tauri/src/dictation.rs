...
use crate::audio::{downmix_into, resample_mono_to_16k, SAMPLE_RATE};
...
fn run_loop(app: AppHandle, model_path: &str, mic: &str, language: &str) -> Result<(), String> {
    ...
    while !STOP.load(Ordering::SeqCst) {
        ...
        if let Some((raw, rate, ch)) = chunk {
            ...
            leftover.extend(resample_mono_to_16k(&mono, rate));
        }
    ...
    if leftover.len() > SAMPLE_RATE as usize / 2 && !STOP.load(Ordering::SeqCst) {
        ...
        if let Ok(text) = transcribe_buffer(&ctx, &leftover, language) {
            if !text.is_empty() {
                let _ = app.emit("dictation_final", serde_json::json!({ "text": text }));
            }
        }
