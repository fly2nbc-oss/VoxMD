...
- **"Free up space" reports the size, not just a count,** and deletes *every* downloaded model — the two ONNX speaker files and the ONNX Runtime library alongside the Whisper weights. A count alone said nothing: two cached models are 0.5 GB or 3 GB depending which two. The row also names the directory the files are in.
...
- **Speaker labels were largely wrong.** Three separate faults, each found by measurement: (a) `segmentation-3.0` was run with non-overlapping windows and every turn cut at the boundary, so 62 % of all speaker changes landed on a multiple of ten seconds — chance is 4 % — and a continuous monologue came out as alternating speakers; it now hops by 50 % and matches each window's arbitrary speaker indices onto the previous one, bringing that to 3 %. (b) The clustering distance sat above the band that separates two voices, merging everyone; measuring real embeddings (same voice 0.07–0.18 apart, different voices 0.55–0.67) put it back inside. (c) Overlapped speech was used for clustering, which blends two voices into one anchor. (d) Clustering ran on long stitched runs, which drift across turn changes — about half of them contained both voices, so the clustering grouped blends. Embeddings are now taken per (window, local speaker), which the segmentation model guarantees to be one speaker. Measured against a reference built from the embedder: on one 12-minute two-person episode **87.9 % of transcript lines get the right speaker** against a 63.6 % majority baseline, up from roughly 62 % before this cycle. On a second episode of the same podcast the score equals its baseline exactly, so the feature is **not yet dependable** — it is off by default and documented as such.
